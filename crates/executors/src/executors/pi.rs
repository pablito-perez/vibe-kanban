...
impl StandardCodingAgentExecutor for Pi {
    async fn spawn(
        ...
        let pi_command = self
            ...
            .build_follow_up(&["--session".to_string(), session_id])?;
...
mod tests {
    ...
    fn pi_follow_up_args_extend_initial() {
        ...
        let (follow_up_program, follow_up_args) = builder
            .build_follow_up(&["--session".to_string(), "session".to_string()])
            .unwrap()
            .into_parts();

        assert_eq!(initial_program, follow_up_program);
        assert!(follow_up_args.starts_with(&initial_args));
        assert!(follow_up_args.ends_with(&["--session".to_string(), "session".to_string()]));
    }
}
